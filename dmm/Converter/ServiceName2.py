...
class ServiceName2(Poll,Converter, object):
	NAME = 0
	NUMBER = 1
	BOUQUET = 2
	PROVIDER = 3
		...
		for line in fmt:
			...
			if f == 't':	# %t - tuner_type (dvb-s/s2/c/t)
				...
				if type == iDVBFrontend.feSatellite:
					...
					result += x in range(4) and {0:'H',1:'V',2:'CL',3:'CR'}[x] or '?'
	...
	def getIPTVProvider(self, refstr):
		...
		elif '4097' in refstr or '8193' in refstr or '8739' in refstr:
			return "StreamTV"
		return ""
